  ...
  try {
    ...
    if (!result.success) {
      return NextResponse.json(
        {
          success: false,
          message: "Invalid reveal request.",
        },
        { status: 400 },
      );
    }
    ...
    await Activity.create({
      ...
      description: `Revealed a secret from credential "${credential.name}"`,
      metadata: {
    ...
    console.error("POST /api/credentials/[credentialId]/reveal error:", error);

    return NextResponse.json(
      {
        success: false,
        message: "Failed to reveal credential secret.",
      },
      { status: 500 },
    );
  }
